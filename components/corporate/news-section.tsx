    ...
    <section className="py-24 lg:py-32 bg-[#f8f7f2] font-sans">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-14">
          {NEWS_META.map((meta, i) => {
            const item = dict.news.items[i]
            return (
              <div key={i} className="group">
                <Link href={`/news/${meta.slug}`} className="block relative aspect-[4/3] rounded-[40px] overflow-hidden mb-8">
                  <img
                    src={meta.img}
                    alt={item.title}
                  ...
                  <div className="absolute top-6 left-6">
                    <span className="inline-block px-4 py-2 text-white text-[11px] font-bold uppercase tracking-wider rounded-xl shadow-lg" style={{ backgroundColor: meta.color }}>
                      {item.category}
                    </span>
                  </div>
                </Link>
