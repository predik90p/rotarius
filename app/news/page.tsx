...
const news = [
  ...
  { id: "swiss-contract", category: "Contract Award", title: "Swiss Armed Forces Contract Extension", excerpt: "Multi-year contract extension to provide surveillance systems and support services through 2030.", date: "February 15, 2026", featured: false, color: '#1C5B68' },
  { id: "ai-navigation", category: "Innovation", title: "AI-Powered Autonomous Navigation System Unveiled", excerpt: "Revolutionary onboard AI enables fully autonomous mission execution in GPS-denied environments.", date: "January 30, 2026", featured: false, color: '#71A58D' },
  { id: "singapore-expansion", category: "Expansion", title: "New Asia-Pacific Service Center Opens in Singapore", excerpt: "Strategic expansion brings full maintenance and support capabilities to Southeast Asian defense partners.", date: "January 20, 2026", featured: false, color: '#2B979F' },
  { id: "defense-award", category: "Recognition", title: "European Defense Innovation Award 2025", excerpt: "Rotarius recognized for breakthrough developments in autonomous swarm coordination technology.", date: "December 10, 2025", featured: false, color: '#1C5B68' },
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="pt-32 pb-16">
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <h1 className="text-5xl lg:text-7xl font-bold tracking-tight text-foreground leading-[1.1]">News</h1>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-8">All News</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {regularNews.map((item) => (
              <Link key={item.id} href={`/news/${item.id}`} className="group bg-white rounded-[32px] border border-border/20 p-8 hover:-translate-y-2 hover:shadow-xl transition-all duration-500">
                <div className="inline-flex items-center gap-2 px-3 py-1 mb-4 rounded-full text-[11px] font-bold uppercase tracking-wider" style={{ backgroundColor: item.color + '15', color: item.color }}>{item.category}</div>
                <h3 className="text-xl font-bold text-foreground leading-tight group-hover:text-primary transition-colors line-clamp-2">{item.title}</h3>
                <p className="mt-3 text-muted-foreground text-sm leading-relaxed line-clamp-2">{item.excerpt}</p>
                <div className="mt-6 flex items-center justify-between">
                  <span className="text-xs text-muted-foreground opacity-60">{item.date}</span>
                  <span className="text-sm font-semibold text-muted-foreground group-hover:text-primary flex items-center gap-2 transition-colors">Read more <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></span>
                </div>
              </Link>
            ))}
